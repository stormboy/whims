...
Whims.prototype.connectionHandler = function(socket) {
	...
	mqtt.createClient(this.mqttPort, this.mqttHost, function(err, mqttClient) {
		if (err) {
			console.log("error while creating MQTT client to " + whims.mqttHost + ": " + err);
			return;
		}
		var connected = false;
		...
		socket.on('subscribe', function(topic) {
			...
			// TODO emit successful subscription
		...
		mqttClient.on('publish', function(packet) {				// got data from subscribed topic
			if (packet.topic.startsWith(clientId)) {			// message is a response to initial-content request
				...
				if (topic) {
					if (TRACE) {
						console.log('MQTT received initial content: ' + topic + ' : ' + packet.payload);
					}
					socket.emit('message', {
						topic : topic,

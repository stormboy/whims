...
MeemBus.prototype.makeRequest = function(topic, query, requestId, cb) {
	...
	this.mqttClient.publish(requestTopic);
	...
	var t = setTimeout(function() {
		...
		self.mqttClient.unsubscribe(responseTopic);
	}, REQ_TIMEOUT);
...
MeemBus.prototype._removeSubscription = function(topic) {
	if (this._subscriptions[topic]) {
		this._subscriptions[topic]--;
		if (this._subscriptions[topic] == 0 && this.mqttClient.connected) {
...
MeemBus.prototype._init = function(mqttClient) {
	...
	mqttClient.addListener('message', function(topic, payload) {
		...
		if (i > 0) {
			...
			self._handleMessage(topic, payload);
		}
	});
}
 ...
 */
MeemBus.prototype._handleMessage = function(topic, payload) {
	this._messageEmitter.emit(topic, payload);
}
...
ContentRequest.prototype.respond = function(message) {
	bus.sendMessage(this.responseTopic, message);
}

/**
  ...
  * - added an unsubscribe function
  * 
	// to read remaining length of message
    var pos = 1;
    var multiplier = 1 
	var len = 0 
	do { 
...
var MQTTSUBSCRIBE = 0x80; //8<<4; non-repeated message
var MQTTUNSUBSCRIBE = 0xA0; //10<<4; on-repeated message
...
MQTTClient.prototype.unsubscribe = function (sub_topic) {
	if(this.connected){
    	var i = 0;
		var buffer = new Buffer(6+sub_topic.length);;
    
		//fixed header
		buffer[i++] = MQTTUNSUBSCRIBE;			// message
		buffer[i++] = 4 + sub_topic.length;		

		//variable header
		buffer[i++] = 0x00;
		buffer[i++] = 0x0b; //message id

		//payload
		buffer[i++] = 0x00;
		buffer[i++] = sub_topic.length; // length of following buffer
...
MQTTClient.prototype.publish = function (pub_topic, payload) {
	...
	if(this.connected && pub_topic && payload) {
        var i = 0, n = 0;
...
/**
 * TODO handle data length differing in size to message length.
 * @param {Object} data
 */
